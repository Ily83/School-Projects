...
vect(1,:) = (/ ( random(), i=1,N ) /) ! abscisse x  
vect(2,:) = (/ ( random(), i=1,N ) /) ! ordonnee y 
! print *,vect(1,:),vect(2,:)
...
! Tracé de points
if (plot == 1) call gp%plot(vect(1,:),vect(2,:), 'title "Points Kouhonen" with points pt 7 ps 1.3 lc rgb "#011582"')
...
subroutine kmeans(N,vect,Ne,plot,integrale)
    ...
    vect = 0_dp; temp = 0_dp; 
    !initialisaiton, On tire N points au hasard
    vect(1,:) = (/ ( random(), i=1,N ) /) ! abscisse x  
    vect(2,:) = (/ ( random(), i=1,N ) /) ! ordonnee y 
    ! Recherce de voisins 
    do r = 1,Ne ! Repetition du processus 50 fois
        do i = 1,N ! boucle sur tous les points
            xa = random();ya = random();
            ...
            do j = 1,N  ! boucles sur les centres j0
                if ( temp(3,j) == i) then 
                    sx = sx + temp(1,j)
                    sy = sy + temp(2,j)
                    cpt = cpt + 1_dp
            ...
            if (cpt /= 0) then ! Moyenne des points
                vect(1,i) = sx/cpt
                vect(2,i) = sy/cpt 
               end if 
        end do 
...
! Tracé de points
if (plot == 1) call gp%plot(vect(1,:),vect(2,:), 'title "Points kmeans" with points pt 7 ps 1.3 lc rgb "#011582"')
!Approximation de l'integrale
integrale = sum(exp(vect(1,:)+vect(2,:)))/N
...
subroutine kmeansquad(N,vect,Ne,plot,integrale)
    ...
    vect(2,:) = (/ ( random(), i=1,N ) /) ! ordonnee y 
    ! Recherce de voisins 
    do r = 1,Ne ! Repetition du processus Ne= 50 fois
        do i = 1,N ! boucle sur tous les points
            xa = random();ya = random();
            j0 = minloc(sqrt( (vect(1,:)-xa)**2 + (vect(2,:)-ya)**2 ),1)
            temp(1,i) = xa
            ...
            temp(4,i) = temp(i,4) + cpt ! Nombre de centres totale
            if (cpt /= 0) then ! Moyenne des points
                vect(1,i) = sx/cpt
                vect(2,i) = sy/cpt 
            end if 
...
! Tracé de points
if (plot == 1) call gp%plot(vect(1,:),vect(2,:), 'title "Points kmeansquad" with points pt 7 ps 1.3 lc rgb "#011582"')
