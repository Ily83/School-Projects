...
real(dp) function G1q(i)
    real(dp) :: U1,U2
    integer :: i
    real(dp), parameter   :: PI = 4_dp*atan(1.0_dp)
    U1 = i*(2.d0**(0.5d0))-int(i*(2.d0**(0.5d0))); 
    U2 =  i*(3.d0**(0.5d0))-int(i*(3.d0**(0.5d0)));
    G1q = sqrt(-2_dp*log(U1))*cos(2_dp*PI*U2)
end function G1q

real(dp) function G2q(i)
    real(dp) :: U1,U2
    integer :: i
    real(dp), parameter   :: PI = 4_dp*atan(1.0_dp)
    U1 = i*(2.d0**(0.5d0))-int(i*(2.d0**(0.5d0))); 
    U2 =  i*(3.d0**(0.5d0))-int(i*(3.d0**(0.5d0)));
    G2q = sqrt(-2_dp*log(U1))*sin(2_dp*PI*U2)
end function G2q
