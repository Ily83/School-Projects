...
function Gaussien3D(U1,U2,U3,U4)
    real(dp), parameter   :: PI = 4_dp*atan(1.0_dp)
    real(dp) :: U1, U2, U3, U4, Gaussien3D(3)
    Gaussien3D(1) = sqrt(-2._dp*log(U1))*cos(2._dp*PI*U2);
    Gaussien3D(2) = sqrt(-2._dp*log(U1))*sin(2._dp*PI*U2);
    Gaussien3D(3) = sqrt(-2._dp*log(U3))*cos(2._dp*PI*U4);
end function Gaussien3D
